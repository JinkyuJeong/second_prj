...
<script type="text/javascript">
		...
		function chgState(data){
			$.each(data, function(i, order){
				...
				if(order.order_state === '결제완료' || order.order_state === '상품준비' || order.order_state === '배송준비' || order.order_state === '배송중'){
					html += '<form action="orderStateChg" method="post" onsubmit="orderStateChg(event, ' + i + ')" style="display:inline-block;">';
				  html += '<input type="hidden" name="order_id" value="' + order.order_id + '">';
				  html += '<input type="hidden" name="order_state" value="' + order.order_state + '">';
				  ...
				  html += '<button type="submit" class="btn btn-dark">네</button>';
				  html += '</div>';
				  html += '</div>';
				  html += '</div>';
				  html += '</div>';
	...
	function orderStateChg(event, i) {
	  event.preventDefault(); 
	
	  const orderId = $('input[name="order_id"]').eq(i).val();
	  const orderState = $('input[name="order_state"]').eq(i).val();
    ...
    <div class="container w3-white pt-1">
      ...
      <div class="container">
      	<form action="orderList">
	        <div style="display: flex;justify-content: space-between;border-bottom: 2px solid black;margin-bottom: 10px;">
	          ...
	          <div class="input-group p-3" style="width: 75%;">
			          ...
			          <option ${param.f eq 'order_id'? 'selected' : '' } value="order_id">주문번호</option>
       ...
       <div class="btn-group mb-3">
				  ...
				  <button type="button" onclick="location.href='orderList?pageNum=${pageNum}&sd=${sd}&ed=${ed}&f=${param.f}&query=${param.query}&order_state=환불대기'" class="btn btn-outline-custom-orange btn-sm ${order_state eq '환불대기' ? 'active-orange' : '' }">환불대기</button>
				  <button type="button" onclick="location.href='orderList?pageNum=${pageNum}&sd=${sd}&ed=${ed}&f=${param.f}&query=${param.query}&order_state=환불완료'" class="btn btn-outline-custom-orange btn-sm ${order_state eq '환불완료' ? 'active-orange' : '' }">환불완료</button>
				</div>
         ...
         <div class="w3-center w3-padding-32">
		    <div class="w3-bar">
			    <c:if test="${pageNum<= 1}">
					...
					<c:if test="${pageNum > 1}">
						<a class="w3-bar-item w3-button w3-hover-black" href="csList?pageNum=orderList?pageNum=${pageNum-1}&sd=${sd}&ed=${ed}&f=${param.f}&query=${param.query}&order_state=${param.order_state}">&laquo;</a>
					</c:if>
					
					<c:forEach var="a" begin="${startPage}" end="${endPage}">
						<c:if test="${a <= maxPage}">
							<a class="w3-bar-item w3-button w3-hover-black ${a == pageNum ? 'w3-black' : '' }" href="orderList?pageNum=${pageNum}&sd=${sd}&ed=${ed}&f=${param.f}&query=${param.query}&order_state=${param.order_state}">${a}</a>
						</c:if>
					</c:forEach>
