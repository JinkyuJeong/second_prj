		    ...
		    <div class="w3-bar">
			    <c:if test="${pageNum<= 1}">
					...
					<c:forEach var="a" begin="${startPage}" end="${endPage}">
						<c:if test="${a <= maxPage}">
							<a class="w3-bar-item w3-button w3-hover-black ${a == pageNum ? 'w3-black' : '' }" href="optList?pageNum=${pageNum-1}&query=${param.query}">${a}</a>
						</c:if>
					</c:forEach>
					...
					<c:if test="${startPage+4 < maxPage}">
						<a class="w3-bar-item w3-button w3-hover-black" href="optList?pageNum=${pageNum-1}&query=${param.query}">&raquo;</a>
					</c:if>
		    </div>
		  </div>
